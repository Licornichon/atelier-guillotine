...
`generate-gallery.js` scans `assets/media/gallery/{battle-ready,tabletop-plus,display}/` (repo root, not `src/`, sub-folders ok; the first sub-folder's name prefix gives the publisher via `PUBLISHERS`, e.g. `w40k-…`/`aos-…`/`mordheim` → `gw`), sorts by its `PRIORITY` list of publisher + level (+ optional game prefix, w40k first) groups, then mtime desc → `gallery.json`. `loaders/pug-with-data.js` prepends `- var galleryItems` + `- var shopItems` to every Pug and marks both JSON as deps. `CopyWebpackPlugin` copies `assets/media/` (minus the photo originals) and `assets/generated/` → `dist/`. One `info.json` per folder holding photos maps each file name to a bare number (no unit: the `gallery.price` key words the currency per language) and adds a caption = level + price per miniature, shown on hover on desktop and under the photo in the lightbox; format doc: `assets/media/gallery/README.md`.
